...
MANIFEST = ROOT / "analysis" / "bbrc_oskm" / "analysis_manifest.yaml"
...
def _build_approved_from_request(
    ...
    *,
    ...
    if decision == "curated_bridge":
        path = overlay_path or BBRC_OVERLAY
        sha = overlay_sha256
        if sha is None:
...
class TestOrthologLossGate(unittest.TestCase):
    ...
    def test_06_approved_curated_bridge_matching_hashes_pass(self):
        self.assertTrue(BBRC_OVERLAY.is_dir())
        genes = [POU5F1, SOX2, KLF4, MYC]
        ...
        species["ortholog_curated_overlay"] = str(BBRC_OVERLAY)
