...
DEFAULT_BBRC_OVERLAY = ROOT / "analysis" / "bbrc_oskm" / "ortholog_policy" / "v1"
DEFAULT_BBRC_AUDIT = ROOT / "analysis" / "bbrc_oskm" / "analysis_manifest.yaml"
...
def resolve_default_overlay_path(
    *,
    ...
    for raw in (session_overlay, yaml_overlay, str(DEFAULT_BBRC_OVERLAY)):
        if not raw:
            continue
...
def resolve_default_audit_path() -> Path | None:
    if DEFAULT_BBRC_AUDIT.is_file():
        return DEFAULT_BBRC_AUDIT.resolve()
    return None
