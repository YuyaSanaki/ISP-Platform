...
from .species_context import default_isp_forward_batch_size
...
def resolve_forward_batch_size(
    ...
    *,
    ...
    fallback = default_isp_forward_batch_size(get_model_input_size(model))
    n_forwards = max(1, int(n_forwards))
    probe_label = label or (
        "ISP forward_batch_size"
        if n_forwards == 1
        else f"ISP forward_batch_size ({n_forwards} forwards)"
    )
    ...
    def probe(batch_size):
        ...
        with torch.no_grad():
            ...
            for _ in range(n_forwards):
                held.append(model(input_ids=input_ids, attention_mask=attention_mask))
            del held
...
def batched_layer_embs(outputs, layer_to_quant):
    """Layer embeddings as [cell, gene, hidden]; a minibatch of one keeps its cell dim."""
...
def forward_pass_single_cell(model, example_cell, layer_to_quant):
    ...
    emb = batched_layer_embs(outputs, layer_to_quant)[0]
    del outputs
    return emb
...
def overexpress_tokens(example):
    """Insert OE genes at the front of the rank-value encoding.
    ...
    for token in example["tokens_to_perturb"][::-1]:
        ids.insert(0, token)
    # Length-preserve: drop lowest-ranked tokens for any net growth from new inserts
    overflow = len(ids) - orig_len
    if overflow > 0:
        if overflow >= len(ids):
            raise RuntimeError(
                f"OE length-preserve would drop entire encoding "
                f"(orig_len={orig_len}, tokens_to_perturb={len(example['tokens_to_perturb'])})"
            )
        del ids[-overflow:]
...
def remove_indices_from_emb(emb, indices_to_remove, gene_dim):
    ...
    indices_to_keep = [i for i in range(emb.size()[gene_dim]) if i not in remove_set]
    num_dims = emb.dim()
    emb_slice = [slice(None) if dim != gene_dim else indices_to_keep for dim in range(num_dims)]
    sliced_emb = emb[tuple(emb_slice)]
    return sliced_emb

def oe_indices_to_remove_for_alignment(idx, orig_max_len, num_perturbed):
    """Build exactly ``num_perturbed`` unique gene indices to drop from the *original* emb.

    Group OE (and length-preserving OE) compares perturbed vs original after stripping
    the OE genes. Absent OE genes were historically padded with the trailing
    ``end_range`` slots. If a present OE gene already sits in that trailing window,
    naive ``idx + end_range`` duplicates indices, so ``remove_indices_from_emb`` drops
    fewer positions for that cell and ``torch.stack`` fails with unequal sizes
    (e.g. [2044, 256] vs [2045, 256]). Fill from the end with indices not already
    selected so every cell removes the same count.
    """
    if num_perturbed <= 0:
        return []
    if idx == [-100] or idx is None:
        selected = []
    else:
        selected = flatten_list(idx) if any(isinstance(el, list) for el in idx) else list(idx)
    seen = set()
    unique = []
    for i in selected:
        if not isinstance(i, int):
            continue
        if i < 0 or i >= orig_max_len or i in seen:
            continue
        seen.add(i)
        unique.append(i)
    j = orig_max_len - 1
    while len(unique) < num_perturbed and j >= 0:
        if j not in seen:
            seen.add(j)
            unique.append(j)
        j -= 1
    if len(unique) < num_perturbed:
        raise RuntimeError(
            f"OE alignment could not select {num_perturbed} unique indices "
            f"(orig_max_len={orig_max_len}, got={len(unique)}, idx={idx!r})"
        )
    return unique[:num_perturbed]
...
def get_cell_state_avg_embs(model,
    ...
    print("Computing state centroids...", flush=True)
    for possible_state in possible_states:
        ...
        state_fbs = forward_batch_size
        if ((total_batch_length-1)/state_fbs).is_integer():
            state_fbs = max(1, state_fbs-1)
        n_batches = max(1, (total_batch_length + state_fbs - 1) // state_fbs)
        print(
            f"State centroid {possible_state}: {total_batch_length} cells, batch={state_fbs} ({n_batches} batches)",
            flush=True,
        )
        running_sum = None
        n_seen = 0
        for b_i, i in enumerate(range(0, total_batch_length, state_fbs), start=1):
            max_range = min(i+state_fbs, total_batch_length)

            state_minibatch = filtered_input_data_state.select([i for i in range(i, max_range)])
            state_minibatch.set_format(type="torch")
            lengths = state_minibatch["length"]
            if hasattr(lengths, "tolist"):
                lengths = lengths.tolist()
            else:
                lengths = list(lengths)
            batch_max_len = min(int(max(lengths)), model_input_size)
            ...
            attention_mask = gen_attention_mask(state_minibatch, batch_max_len)
            ...
            lens_t = torch.as_tensor(lengths, dtype=torch.float, device=hidden.device)
            # Pool immediately and move to CPU so hidden-state storage is not retained.
            pooled = mean_nonpadding_embs(hidden, lens_t).detach().float().cpu().contiguous()
            if running_sum is None:
                running_sum = pooled.sum(dim=0)
            else:
                running_sum += pooled.sum(dim=0)
            n_seen += int(pooled.size(0))
            del outputs
            del hidden
            del pooled
            del state_minibatch
            ...
            del attention_mask
            del input_ids
            if b_i == 1 or b_i == n_batches or b_i % 20 == 0:
                print(f"  {possible_state} centroid batch {b_i}/{n_batches}", flush=True)
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()

        if running_sum is None or n_seen == 0:
            raise RuntimeError(f"No embeddings for centroid state {possible_state!r}")
        avg_state_emb = (running_sum / float(n_seen)).unsqueeze(0).to(ISP_device)
...
def quant_cos_sims(model, 
    ...
    if ((total_batch_length-1)/forward_batch_size).is_integer():
        forward_batch_size = max(1, forward_batch_size-1)
    ...
    for i in range(0, total_batch_length, forward_batch_size):
        ...
        if perturb_group == True:
            ...
            original_minibatch_emb = batched_layer_embs(original_outputs, layer_to_quant).clone()
            del original_outputs

            # Align original emb length to the OE-stripped perturbed emb.
            # Absent OE genes are filled from the trailing ranks; always select
            # exactly len(tokens_to_perturb) *unique* indices (see helper) so
            # torch.stack does not see per-cell length drift from end_range overlap.
            num_perturbed = len(tokens_to_perturb)
            indices_to_perturb_minibatch = [
                oe_indices_to_remove_for_alignment(idx, orig_max_len, num_perturbed)
...
def _align_emb_ranks(original_emb, minibatch_emb):
    """Restore the leading cell dim if either side arrived as a bare [gene, hidden]."""
    if original_emb.dim() == 2:
        original_emb = original_emb.unsqueeze(0)
    if minibatch_emb.dim() == 2:
        minibatch_emb = minibatch_emb.unsqueeze(0)
    return original_emb, minibatch_emb
...
def cos_sim_shift(original_emb, 
    ...
    cos = torch.nn.CosineSimilarity(dim=2)
    original_emb, minibatch_emb = _align_emb_ranks(original_emb, minibatch_emb)
    if original_emb.size() != minibatch_emb.size():
        ...
        raise RuntimeError(
            f"Embedding shape mismatch: original={original_emb.size()}, "
            f"minibatch={minibatch_emb.size()}"
        )
    ...
    shift = perturb_v_end - origin_v_end
    if shift.dim() == 0:
        # A minibatch of one squeezes down to a scalar; torch.cat needs at least 1-D.
    ...
    def __init__(
        ...
        perturb_type : {"delete","overexpress","inhibit","activate"}
            ...
            "overexpress": move gene to front of rank value encoding
                (absent genes are inserted at front; net length growth is offset by
                dropping the lowest-ranked / tail tokens so encoding length is preserved)
        ...
        forward_batch_size : int or "auto"
            Batch size for forward pass. "auto" measures this GPU at startup.
        nproc : int
        ...
        if self.perturb_type in ["inhibit", "activate"]:
            if (self.perturb_rank_shift is None) and (self.perturb_rank_direct_shift is None) :
                if self.perturb_type == "inhibit":
                    ...
                    logger.warning(
                            f"please set the perturb_rank_direct_shift set into 1-{self.model_input_size}. " )
                elif self.perturb_type == "active":
                    logger.warning(
                        "please set the perturb_rank_shift into 1-4. " )
                    logger.warning(
                            f"please set the perturb_rank_direct_shift into 1-{self.model_input_size}. " )
                logger.warning(
