...
def _resolve_gene_name_id_dict() -> Path:
    for name in ("MLM-re_token_dictionary_v1_GeneSymbol_to_EnsemblID.pkl",):
        for base in (_DICTS_DIR / "mouse", _DICTS_DIR):
            path = base / name
            if path.is_file():
                return path
    return _DICTS_DIR / "mouse" / "MLM-re_token_dictionary_v1_GeneSymbol_to_EnsemblID.pkl"


GENE_NAME_ID_DICTIONARY_FILE = _resolve_gene_name_id_dict()
