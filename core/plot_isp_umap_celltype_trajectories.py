...
def run_celltype_trajectory_plots(
    ...
    *,
    ...
    pal = palette_for_groups(labels, resolved_group)
    order = order_for_groups(labels, resolved_group)
    order = [o for o in order if o in set(labels)]

    summary = build_celltype_shift_summary(df, resolved_group, toward_col=toward_col)
