...
1. **Cell-type prediction** — when postprocess cell-type is on, **pre-ISP platform labels** (`cell_type` written at tokenize with `celltype_annotator=isp_expression_v2`, or an existing `isp_expression_v1` dataset) are used (`prefer_metadata_celltype: auto`). Brain token-rank marker panels were removed in v1.3.0; without platform labels cells stay `Unknown`. Raw user-filled `cell_type` without platform provenance is still ignored unless `prefer_metadata_celltype: true`.
2. **Joint UMAP overlays** — `umap_joint_l2_cluster_celltype.png` (+ enriched CSV, `joint_umap_coords.npy`).
3. **Cell-type trajectory tracking** — `umap_celltype_trajectories.png` (arrows + mean displacement vectors colored by predicted cell type), `celltype_shift_summary.csv`, `l2_mean_by_pred_celltype.png` (bars ordered by mean `shift_l2` descending; Unknown/Ambiguous omitted; types with fewer than 2 cells omitted so singleton false calls like Neuron do not appear), and `l2_mean_by_pred_celltype_n_gt20.png` (same mean bar restricted to types with n>20). Coarse L2 figures are no longer written.
...
- tokenize (`tokenizer.celltype_annotation`, default on) scores [`isp_expression_v2.json`](../core/geneformer/dicts/celltype_panels/isp_expression_v2.json) on the count matrix before loom write (`core/celltype_annotate_expression.py`). Each type has core / support / anti markers. A label requires the core score, a minimum number of detected core genes (2, or 3 for erythroid, megakaryocyte, and choroid plexus), and margin over the runner-up. Two different compartments both clearing the score gate are labeled `Doublet_suspected` instead of a cell type. `Epithelial_general` is a lineage gate and is not assigned. v1 ([`isp_expression_v1.json`](../core/geneformer/dicts/celltype_panels/isp_expression_v1.json)) remains for already tokenized runs.
- Labels (`cell_type`, `tissue`, `celltype_score`, `celltype_margin`, `celltype_core_hits`, `celltype_runner_up`, `celltype_doublet_flag`, `celltype_state`, `celltype_annotator=isp_expression_v2`) propagate into the HF dataset. `celltype_state` holds programs that are not competing cell types (skeletal `fiber_program`, plus `mural_program` / `myeloid_program` alternates). `Epithelial_general` only adds a positive score to epithelial types. Each type logs `tokenized_marker_count/registered_marker_count`; `celltype_confidence=low` when fewer than half of its core genes are in the Geneformer vocabulary. Stromal types and lymphatic endothelium require 3 detected core genes. `C1QC_APOE_macrophage` is used only when a myeloid gate passes and microglia core is low; a neural/brain prior (`organ_major`, `organ`, `tissue_prior`, `sample_organ`, or `tissue`) blocks that label.
