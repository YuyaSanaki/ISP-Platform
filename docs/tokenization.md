...
```yaml
tokenizer:
  ortholog_audit: /app/analysis/bbrc_oskm/analysis_manifest.yaml
...
docker compose run --rm pipeline python3 /app/core/execute_tokenizer_pipeline.py \
  --ortholog-audit /app/analysis/bbrc_oskm/analysis_manifest.yaml
...
docker compose run --rm pipeline python3 /app/core/execute_tokenizer_pipeline.py \
  ...
  --ortholog-curated-overlay /app/analysis/bbrc_oskm/ortholog_policy/v1   # needed for curated_bridge
...
See BBRC `analysis/bbrc_oskm/analysis_manifest.yaml` → `ortholog_audit`.
