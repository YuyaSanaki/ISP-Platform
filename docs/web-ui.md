...
| Cluster / cell-type analysis      | `stages.isp.postprocess.enabled` (+ `n_clusters`, `celltype_prediction`; default **off**). When on, prefers pre-ISP platform `cell_type` (`prefer_metadata_celltype: auto`). |


**Trajectory UMAP** (per-cell arrows): use Run type **ISP UMAP**. Choose a past E2E pipeline folder + gene; toggle **Draw trajectory lines** (and arrow count) under Plot options. Optional expander **Cluster / cell-type analysis** enables `cluster_coexpr_analysis/` after the UMAP (`n_clusters` can be a fixed K or `auto`). Cell-type labels use tokenize-time platform annotation when present; otherwise marker panels on tokens. The job runs `run_isp_umap.py --run-dir … --gene …` and writes under `{pipeline_run}/isp_umap/`.
